...
import { validateTileOverlaps } from './game/utils/tile-overlap-validator.js';
...
function buildMenuSubConfig(menuConfig, screenKey, colorsConfig) {
  ...
  return {
    ...
    'subtitle-key': screen['subtitle-key'] ?? null,
    'hint-key': screen['hint-key'] ?? null,
    ...
    subtitle: resolveConfig(subtitleDefaults, colorsConfig),
    hint: resolveConfig(hintDefaults, colorsConfig),
...
async function bootstrap() {
  ...
  const audioEngine = new AudioEngine(audioConfig, audioSfxConfig, audioBgmConfig, noteCalculator, audioTuningConfig['default-envelope']);
  ...
  const noteRangeConfig = {
    lowest: noteScale[0],
    count: noteScale.length
  };
  const noteCollection = new NoteCollectionTracker(noteRangeConfig);

  const inventoryConfig = gameConfig.inventory;
  const inventory = new Inventory(inventoryConfig, audioEngine);
  ...
  player.audioEngine = audioEngine;
  ...
  const projectileConfig = resolveConfig(gameConfig.projectile, colorsConfig);
  ...
  function createStageFromData(stageDataId) {
    ...
    for (const section of stageConfig.sections ?? []) {
      ...
      for (const obj of section.objects ?? []) {
        ...
        if (isTile) {
          const tileConfig = resolveTileType(obj, tileDataMap, colorsConfig, bordersConfig);
    ...
    validateTileOverlaps(stage.tilesList, collisionSolver);
